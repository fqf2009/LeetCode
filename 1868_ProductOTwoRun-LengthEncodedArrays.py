...
class Solution:
    def findRLEArray(self, encoded1: List[List[int]], 
        ...
        while True:
            ...
            num3, freq3 = num1*num2, min(freq1, freq2)
            freq1 -= freq3
            freq2 -= freq3

            if not res or res[-1][0] != num3:
                res.append([num3, freq3])
            else:
                res[-1][1] += freq3
...
if __name__ == '__main__':
    def unit_test(sol):
        r = sol.findRLEArray([[1,3],[2,3]], [[6,3],[3,3]])
        print(r)
        assert r == [[6,6]]

        r = sol.findRLEArray([[1,3],[2,1],[3,2]], [[2,3],[3,3]])
        print(r)
        assert r == [[2,3],[6,1],[9,2]]
