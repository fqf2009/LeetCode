...
# DP - T/S: O(n*k*k), O(n*k), n = len(piles)
# - dp[i, k]: pick k elements from pile[i] to pile[n-1]
class Solution:
    def maxValueOfCoins(self, piles: List[List[int]], k: int) -> int:
        ...
        def dp(i, k) -> int:
            ...
            for j in range(min(len(piles[i]), k)):
                pileValue += piles[i][j]
                res = max(res, pileValue + dp(i+1, k-j-1))
            return res
...
class Solution1:
    def maxValueOfCoins(self, piles: List[List[int]], k: int) -> int:
        ...
        def backtrack(pileIdx, coinsPicked, totalValue):
            ...
            backtrack(pileIdx + 1, coinsPicked, totalValue) # pick 0 coin from this pile
            for val in piles[pileIdx]:  # pick 1, 2, ... coins from this pile
                coinsPicked += 1
                totalValue += val
...
if __name__ == "__main__":
    ...
    def unitTest(sol):
        r = sol.maxValueOfCoins([[1, 100, 3], [7, 8, 9]], k=2)
        print(r)
        assert r == 101

        r = sol.maxValueOfCoins([[100], [100], [100], [100], [100], [100], [1, 1, 1, 1, 1, 1, 700]], k=7)
        print(r)
        assert r == 706

        piles = [
            [72439, 85695, 81846, 42758, 65144, 14297, 31537, 39451],
            [
                19764,
                71823,
                36354,
        ...
        r = sol.maxValueOfCoins(piles, 307)
        print(r)
        assert r == 17035711
