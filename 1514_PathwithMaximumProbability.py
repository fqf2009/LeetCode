...
# - Both need to store info of the known set of nodes (from edges actually), then retrieve the
#   lowest priority value item, and then use it to visit the un-visited set of nodes. So both
#   can use PriorityQueue to save storing and retrieving time: From N to log(N), where N is queue length.
# - Both keep a set of visited nodes to avoid re-visiting.
# - Dijkstra algorithm keeps the distance (cost) from source node to each queued node. Each item it adds
#   an edge (node1, node2) into queue, it adds the distince of source to node1, with the distance of
#   node1 to node2. When it pop the edge from queue, it gets the node with the lowest cost by far.
# - Prim's algorithm keeps the distance of node1 to node2 when adding the edge to the queue. The meaning
#   is from the set of visited nodes to the set of unvisited nodes, pick shortest edge for next visiting.
...
# traversing that edge succProb[i]. Given two nodes start and end, find the path with the maximum probability
